  ...
  return (
    <nav className={`fixed top-0 w-full z-50 transition-all duration-500 ${scrolled ? "bg-bg/90 backdrop-blur-xl" : "bg-transparent"}`}>
      <div className="max-w-7xl mx-auto px-6 lg:px-10 h-20 flex items-center justify-between">
        <Link href="/" className="font-serif text-2xl font-bold italic">Kuldeep Saini</Link>
        <div className="hidden md:flex items-center gap-8">
          {links.map((l) => (
            <a key={l.href} href={l.href} className="text-sm text-white hover:text-white/70 transition-colors duration-300">{l.label}</a>
          ))}
        </div>
      ...
      {open && (
        <div className="md:hidden bg-bg border-t border-white/5 px-6 pb-8 pt-4">
          {links.map((l) => (
            <a key={l.href} href={l.href} onClick={() => setOpen(false)} className="block py-3 text-white hover:text-white/70 transition-colors text-sm border-b border-white/5">{l.label}</a>
          ))}
